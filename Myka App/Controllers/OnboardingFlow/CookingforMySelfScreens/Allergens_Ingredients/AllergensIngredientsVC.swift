...
class AllergensIngredientsVC: UIViewController, UITextFieldDelegate {
    ...
    var AllergensIngredients = [ModelClass]()
...
//        ]
        ...
        if self.type == "MySelf"{
            self.ProgressLbl.text = "5/10"
            let progressVw = Float(5) / Float(10)
            progressView.progress = Float(progressVw)
  
            self.SubTitleLbl.text = "Search and select the ingredients you’re allergic to, so I can help you avoid them"
             
        }else if self.type == "Partner"{
            self.ProgressLbl.text = "5/11"
            let progressVw = Float(5) / Float(11)
            progressView.progress = Float(progressVw)
 
            self.SubTitleLbl.text = "Search and select the ingredients you and your partner are allergic to, so I can help you avoid them"
        }else{
            self.ProgressLbl.text = "5/11"
            let progressVw = Float(5) / Float(11)
            progressView.progress = Float(progressVw)
 
            self.SubTitleLbl.text = "Which ingredients are you and your family allergic to?"
        }
        ...
        if comesfrom == ""{
            self.Api_To_GetAllergensIngredients()
        }else{
    ...
    @objc func TextSearch(sender: UITextField){
        if self.SearchTxt.text == ""{
            self.AllergensIngredients.removeAll()
            moreCount = 5
...
//    }
        ...
        @IBAction func SkipBtn(_ sender: UIButton) {
            ...
            vc.backAction = {
                ...
                StateMangerModelClass.shared.onboardingSelectedData.MySelfSeldata[0].AllergensIngredients.removeAll()
    ...
    extension AllergensIngredientsVC: UITableViewDelegate, UITableViewDataSource {
        ...
        func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            var AllergensIngredientsArr = [String]()
            
            for i in 0..<ArrData.count {
                if ArrData[i].isSelected {
                    AllergensIngredientsArr.append("\(ArrData[i].id ?? Int())")
                }
            }
...
//                self.TblV.reloadData()
        ...
        // MARK: - Draft Restore
        private func restoreDraftAllergensIngredients() {
            guard
                comesfrom == "",
                let saved = StateMangerModelClass.shared.onboardingSelectedData.MySelfSeldata.first
            else { return }

            let savedIds = saved.AllergensIngredients
        ...
        // MARK: - Draft Save
        private func saveDraftAllergensIngredients() {
            ...
            StateMangerModelClass.shared.onboardingSelectedData.MySelfSeldata[0].AllergensIngredients = selectedIds
        }
    }



extension AllergensIngredientsVC {
    func Api_To_GetAllergensIngredients(){
...
//        params["device_token"] = token
        ...
        let loginURL = baseURL.baseURL + appEndPoints.allergensIngredients + "/\(moreCount)/\(self.SearchTxt.text!)"
        print(params,"Params")
...
//                self.AllergensIngredients.removeAll()
                if self.SearchTxt.text == ""{
                    self.AllergensIngredients.append(contentsOf: ModelClass.getBodyGoalsDetails(responseArray: responseArray))
                    //                self.AllergensIngredients = ModelClass.getBodyGoalsDetails(responseArray: responseArray)
                    
                    // Remove duplicates by name
                    var seenNames = Set<String>()
                    self.AllergensIngredients = self.AllergensIngredients.filter {
                    ...
                    let priorityOrder = ["dairy", "egg", "peanut", "tree nut", "shellfish"]
                    ...
                    for name in priorityOrder {
                        if let item = self.AllergensIngredients.first(where: { $0.name.lowercased() == name }) {
                    ...
                    for item in self.AllergensIngredients {
...
//                    self.DislikesIngredientArr.append(contentsOf: ModelClass.getBodyGoalsDetails(responseArray: responseArray))
                    ...
                    for i in self.AllergensIngredients{
...
//                }
                ...
                if self.SearchTxt.text?.count ?? 0 == 0{
                    if self.ArrData.count > self.moreCount{
                ...
                self.TblV.reloadData()
                self.restoreDraftAllergensIngredients()
            }else{
...
extension AllergensIngredientsVC {
    func Api_To_GetPrefrenceBodyGoals(){
        var params = [String: Any]()
        params["allergic_num"] = "\(self.moreCount)"
        params["allergic_search"] = self.SearchTxt.text!
        ...
        WebService.shared.postServiceURLEncoding(loginURL, VC: self, andParameter: params, withCompletion: { (json, statusCode) in
            ...
            if dictData["success"] as? Bool == true{
                ...
                let responseArray = result["allergesingredient"] as? [[String : Any]] ?? [[String: Any]]()
...
//                }
    ...
    func Api_To_UpdatePrefrence(){
        ...
        params["allergies"] = AllergensIngredientsArr

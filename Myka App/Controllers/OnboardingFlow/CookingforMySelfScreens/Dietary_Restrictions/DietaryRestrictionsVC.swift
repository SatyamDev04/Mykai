...
class DietaryRestrictionsVC: UIViewController {
...
@IBOutlet weak var ProgressLbl: UILabel!
    ...
    var DietaryRestrictionsArr = [ModelClass]()
    ...
    override func viewDidLoad() {
        ...
        if self.type == "MySelf"{
            self.ProgressLbl.text = "2/10"
            let progressVw = Float(2) / Float(10)
            progressView.progress = Float(progressVw)
            
            let helloString = NSAttributedString(string: "Dietary", attributes: Attributes1)
            let worldString = NSAttributedString(string: " Restrictions", attributes: Attributes2)
            let fullString = NSMutableAttributedString()
            fullString.append(helloString)
            fullString.append(worldString)
            self.TitleLbl.attributedText = fullString
            self.SubTitleLbl.text = "Do you have any dietary restrictions?"
             
        }else if self.type == "Partner"{
            self.ProgressLbl.text = "3/11"
            let progressVw = Float(3) / Float(11)
            ...
            fullString.append(worldString)
            self.TitleLbl.attributedText = fullString
            self.SubTitleLbl.text = "Do you or your partner have any dietary restriction?"
        }else{
            self.ProgressLbl.text = "3/11"
            let progressVw = Float(3) / Float(11)
            progressView.progress = progressVw
            ...
            fullString.append(worldString)
            self.TitleLbl.attributedText = fullString
            self.SubTitleLbl.text = "Do you or any of your family members have \nany  dietary restrictions?"
        }
         
        if comesfrom == ""{
            self.Api_To_GetDietaryRestrictions()
        }else{
    ...
    @IBAction func SkipBtn(_ sender: UIButton) {
        ...
        vc.backAction = {
            ...
            StateMangerModelClass.shared.onboardingSelectedData.MySelfSeldata[0].DietaryPreferences.removeAll()
...
extension DietaryRestrictionsVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        var SelDietaryArr = [String]()
        
        for i in 0..<ArrData.count {
            if ArrData[i].isSelected {
                SelDietaryArr.append("\(ArrData[i].id ?? Int())")
            }
        }
...
extension DietaryRestrictionsVC {
    func Api_To_GetPrefrenceBodyGoals(){
        ...
        WebService.shared.postServiceURLEncoding(loginURL, VC: self, andParameter: params, withCompletion: { (json, statusCode) in
            ...
            if dictData["success"] as? Bool == true{
                ...
                let responseArray = result["dietaryrestriction"] as? [[String : Any]] ?? [[String: Any]]()
    ...
    func Api_To_UpdatePrefrence(){
        ...
        params["dietary_id"] = SelDietaryArr

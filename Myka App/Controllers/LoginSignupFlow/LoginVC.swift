...
class LoginVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let email_phone = UserDetail.shared.getUserEmail_Phone()
        let pass = UserDetail.shared.getUserPass()
        
        if email_phone != "" && pass != "" {
            self.Email_PhoneTxt.text = email_phone
    ...
    @IBAction func ForgetBtn(_ sender: UIButton) {
        ...
        let vc = storyboard.instantiateViewController(withIdentifier: "FoergetVC") as! FoergetVC
    ...
    @IBAction func AppleLogiinBtn(_ sender: UIButton) {
        appleSignIn.didTapLoginWithApple1 { (userInfo, message) in
            if let userInfo = userInfo{
                ...
                let Lastname = userInfo.lastName != "" ? userInfo.lastName : "Test"
    ...
    func validation() -> Bool {
        ...
        if Email_PhoneTxt.text?.count == 0 {
            self.popupAlert(title: "Error", message: "Email can't be empty.", actionTitles: ["Okay!"], actions:[{action1 in}])
            return false
        }
        else if Email_PhoneTxt.text?.isNumeric == false {
            if  !Email_PhoneTxt.text!.isValidEmail(){
                self.popupAlert(title: "Error", message: "Please enter valid email.", actionTitles: ["Okay!"], actions:[{action1 in}])
                return false
            }
    ...
    func Api_To_login(){
        ...
        WebService.shared.postServiceURLEncoding(loginURL, VC: self, andParameter: params, withCompletion: { (json, statusCode) in
            ...
            if dictData["success"] as? Bool == true{
                ...
                let username = self.Email_PhoneTxt.text ?? ""
                let password = self.PassTxt.text ?? ""

                if self.RememberMeBtn.isSelected == true{
                    if self.Email_PhoneTxt.text?.isNumeric == true || self.Email_PhoneTxt.text!.count == 10 {
                        let p = self.Email_PhoneTxt.text ?? ""
                        let k = p.removeSpaces
                        let o = k.replace(string: "(", withString: "")
                        let t = o.replace(string: ")", withString: "")
                        ...
                        UserDetail.shared.setUserEmail_Phone("\(numb)")
                    }else{
                        UserDetail.shared.setUserEmail_Phone(username)
                    }
                    
                    UserDetail.shared.setUserPass(password)
                }
                //
